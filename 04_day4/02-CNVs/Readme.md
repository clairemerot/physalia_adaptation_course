...
First you need to move within the folder ``04_day4/02-CNVs/``. To make it easier, I added all required files:
* 00-scripts, you will find all the scripts that we will use to extract CNVs info. Note that all these scripts came from the stacks_workflow developed by Eric Normandeau.
* 01-info_files, I added a file which contain all samples informations (sample ids, pop, sex, temperature...)
* 02-data, I added a raw VCF file, generated by stacks using lax parameters

**NOTE:** Visite the stacks_workflow github repository ---> https://github.com/enormandeau/stacks_workflow
...
To do so, we will use the custom python script ``01-filter_vcf_fast.py`` and set the following options:
...
Nevertheless, we will do this step at your local computer, where it should be easier to display the figures.

So, in your local working folder, please download the following two files:
* 03-analyses/capelin_canada_filtered_4_60_0_6_overmerged_loci.txt
* 00-scripts/03.0_snp_categorization.R

#### [On your local computer]
Run the Rscript 03.0_snp_categorization.R.
```
