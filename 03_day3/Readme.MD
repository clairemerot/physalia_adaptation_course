...
####  prepare the data
Let's open R in our terminal (just type "R") and convert our vcf to the outflank format:
You may prefer to run it locally on your computer, feel free to do so but make sure you copy all the 03_day3 folder to keep using the same paths.
...
We want to pinpoint the outlier that differ from the basic structure of the population. This is not a big deal in our example but it may be in other system. 
...
```
On the server we have set up everything for you but if you try on your computer, you may have an error because it does not find Baypass
You can set a path variable ```PATH_TO_SOFT=WhereItIs/baypass_2.2/sources``` and then do ```$PATH_TO_SOFT/g_baypass```, or add the path to your .bashrc
...
If we don't want to control for population structure, we could have run it directly (please do not run now ) ``` $PATH_TO_SOFT/g_baypass -npop 12 -gfile 05_baypass/canada.baypass -outprefix 05_baypass/allsnps.output -nthreads 1 ```
...
[simulate_baypass](https://github.com/clairemerot/physalia_adaptation_course/blob/2021/03_day3/00_documents/simulate_baypass.md) 

```
#controlled run on the simu dataset
$PATH_TO_SOFT/g_baypass -npop 12 -gfile 05_baypass/G.simulates_pods \
-omegafile 05_baypass/prunedsnps.output_mat_omega.out \
...
echo "$seed"
$PATH_TO_SOFT/g_baypass -npop $N_pop -gfile $geno -efile $env -omegafile $omega_mat -outprefix output"$i" -nthreads $N_CPU -npilot 25 -burnin 5000 -seed $seed
done
```
...
Today we will be interested in using temeprature. I have used the database MarSPEC to extract it. bioOracle is also a good option, as well as WorldClim.
Here is a script that may be useful is you want to do it yourself
[extract clim variables](https://github.com/clairemerot/physalia_adaptation_course/blob/2021/03_day3/tuto_worldclim.R)

Baypass wants the environment as one line per env variable and one column per population. no header.
I have used the file 02_data/info_pop_geo_eco.txt to format it in R. This is the file 05_baypass/env.txt.
We also need to make really sure that our env file is exactly in the same order as the population are in the geno file.
...
We can export the list of outlier SNPs for subsequent analysis on day 5. Here is the code for controlled models .
```
#load bf values
...
We will do most of the computational aspect of the RDA on the server (in R) but you could probably also run it locally, it is usually quite fast.
...
temp.signif.full
```
so you see that temperature does not explain a large fraction of total variance (0.1%!!) but the model appear significant. It likely means that very few loci covary with temperature but this parameter does not explain a large fraction of genetic variation as a whole. This is expected as most SNPs will likely be neutral.
...
In our system in which gene flow is super high and geographic structure very low, it is not recommended to control the rda by geography or neutral structure. Yet, as a proff of principle, you can explore how to do it.

[Complementary RDA tutorial](https://github.com/clairemerot/physalia_adaptation_course/blob/2021/03_day3/00_documents/RDA.md)
